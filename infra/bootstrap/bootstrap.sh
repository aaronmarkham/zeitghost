...
#   HOSTNAME_OVERRIDE  Default: keep current hostname.
#
# Usage:
...
default_phase() {
    ...
    log "2/8  Create deploy user '$DEPLOY_USER' with sudo NOPASSWD"
    if ! id "$DEPLOY_USER" >/dev/null 2>&1; then
    ...
    log "3/8  Install authorized_keys for $DEPLOY_USER"
    USER_HOME=$(getent passwd "$DEPLOY_USER" | cut -d: -f6)
    ...
    log "4/8  Tailscale install + auth"
    ...
    if ! tailscale status >/dev/null 2>&1 || \
       ! tailscale status --json 2>/dev/null | grep -q '"BackendState":"Running"'; then
        ...
        TS_HOSTNAME="${HOSTNAME_OVERRIDE:-$(hostname)}"
        tailscale up --authkey="$TAILSCALE_AUTHKEY" --ssh \
                     --hostname="$TS_HOSTNAME" --accept-dns=false
        log "    Tailscale up as $TS_HOSTNAME"
    else
        log "    Tailscale already running"
    fi

    log "5/8  Docker engine + compose plugin (official repo)"
    ...
    log "6/8  UFW firewall (allow ssh, 80, 443; deny everything else inbound)"
    ...
    log "7/8  Unattended-upgrades for security patches"
...
EOF
    ...
    log "8/8  Swap ($SWAP_GB GB) if none configured"
